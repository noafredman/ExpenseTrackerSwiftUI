...
final class DataManager: DataManagerProtocol {
    ...
    func getCurrentUserName() -> String? {
        if let str = userDefaults.data(forKey: UserDefaultsKeys.currentUserName.rawValue) {
            return try? JSONDecoder().decode(String.self, from: str)
        }
        return nil
    }
            ...
            if var decodedUsersInfo = try? JSONDecoder().decode([UserInfo].self, from: savedUserInfo) {
                if let savedUserInfo = decodedUsersInfo.first (where: { $0.name == name}) {
                    ...
                    if let encodedUserId = try? savedUserInfo.id.encode(),
                       let encodedUserName = try? userInfo.name.encode(){
                        userDefaults.set(encodedUserId, forKey: UserDefaultsKeys.currentUserId)
                        userDefaults.set(encodedUserName, forKey: UserDefaultsKeys.currentUserName)
                    } else {
    ...
    func handleLogout() {
        userDefaults.removeObject(forKey: UserDefaultsKeys.currentUserName.rawValue)
    }
}
